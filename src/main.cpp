...
{
    std::cout << "\t\t==========================================================" << std::endl;
    std::cout << "\t\t\t   ≣≣≣≣≣≣≣≣≣≣" << player.GetName() << "'s Portfolio:"
              << "≣≣≣≣≣≣≣≣≣≣" << std::endl;
...
void PrintStockLists(const vector<std::shared_ptr<Stock>> &stocks)
{
    ...
    for (auto &s : stocks)
    {
        std::cout << "\t" << i << ". " << s.get()->GetSymbol() << ":"
                  << " $" << s.get()->GetCurrentPrice() << std::endl;
        i++;
    }
}
...
{
    // price of the stock
    double stockPrice = (stocks.at(stockIndex)->GetCurrentPrice());
